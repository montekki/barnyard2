...
int Barnyard2Main(int argc, char *argv[])
{
    ...
    if (((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo.state & WALDO_STATE_ENABLED)
    {
	if( (spoolerOpenWaldo(&((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo)))
	{
	    /* XXX */
	    return 1;
	}
	
        if( (spoolerReadWaldo(&((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo)))
	{
	    /* XXX */
	    CleanExit(1);
	}
	...
	LogMessage("Using waldo file '%s':\n"
		   ...
		   "    time_stamp	    = %lu\n"
		   "    record_idx	    = %lu\n", 
		   ((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo.filepath,
		   ((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo.data.spool_dir,
		   ((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo.data.spool_filebase,
		   ((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo.data.timestamp,
		   ((Unified2InputPluginContext *)barnyard2_conf->input_configs->context)->waldo.data.record_idx);
    }
    ...
    {
	ProcessContinuousWithWaldo(barnyard2_conf->input_configs);
    }
...
static void ParseCmdLine(int argc, char **argv)
{
    ...
    */
	...
	switch (ch)
	{
		...
		ConfigSpoolDirectory(&bc->tempWaldo, optarg);
		break;
	    ...
	    case 'f':
		bc->run_mode_flags |= RUN_MODE_FLAG__CONTINUOUS;
		ConfigSpoolFilebase(&bc->tempWaldo, optarg);
		break;
	...
	case 'w':
	    bc->run_mode_flags |= RUN_MODE_FLAG__CONTINUOUS;
	    ConfigWaldoFile(&bc->tempWaldo, optarg);
	    break;
...
if (cmd_line->tempWaldo.data.spool_filebase[0] != '\0')
{
    ConfigSpoolFilebase(&((Unified2InputPluginContext *)config_file->input_configs->context)->waldo, 
			cmd_line->tempWaldo.data.spool_filebase);
}

if (cmd_line->tempWaldo.data.spool_dir[0] != '\0')
{
    ConfigSpoolDirectory(&((Unified2InputPluginContext *)config_file->input_configs->context)->waldo, 
			 cmd_line->tempWaldo.data.spool_dir);
}

if (cmd_line->tempWaldo.filepath[0] != '\0')
{
    ConfigWaldoFile(&((Unified2InputPluginContext *)config_file->input_configs->context)->waldo, 
		    cmd_line->tempWaldo.filepath);
}
